export function setOutputContent(html) {
...
export function renderScheduleResult(result) {
    ...
    const perGroupItems = Object.entries(result.per_group || {})
        ...
        events.forEach((event) => {
            ...
            dayGroups[event.day].push(`<div class="item">${escapeHtml(event.start_str)} - ${escapeHtml(event.end_str)} : ${escapeHtml(event.activity)}</div>`);
        });
    ...
    const perActivityItems = Object.entries(result.per_activity || {})
        ...
        events.forEach((event) => {
            ...
            dayGroups[event.day].push(`<div class="item">${escapeHtml(event.start_str)} - ${escapeHtml(event.end_str)} : ${escapeHtml(event.group)}</div>`);
        });
        const daySections = Object.entries(dayGroups)
            .map(([day, items], index) => `
          <div class="day-section">
